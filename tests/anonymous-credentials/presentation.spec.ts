...
describe(`${Scheme} Presentation creation and verification`, () => {
  ...
  // generated by prover:
  let proverSecretKey1: Uint8Array;
  ...
  let attributeNames1: Map<number, Set<string>> = new Map();
  ...
  beforeAll(async () => {
    ...
    proverSecretKey1 = generateRandomFieldElement();
  ...
  it('from credential1 - presenting pseudonyms', () => {
    ...
    const builder1 = new PresentationBuilder();
    expect(builder1.addCredential(credential1, pk1)).toEqual(0);
    attributeNames1.set(0, new Set(['credentialSubject.SSN']));
    expect(
    ...
    const basesForAttributes = PseudonymBases.decodeBasesForAttributes(bases1ForAttributes);
    const baseForSecretKey = PseudonymBases.decodeBaseForSecretKey(base1ForSecretKey);
    const schema = credential1.schema;
    const attributeSSN = schema.encoder.encodeMessage(`${SUBJECT_STR}.SSN`, credential1.subject['SSN']);
    ...
    expect(pres1.spec.boundedPseudonyms.size).toEqual(1);
    expect(pres1.spec.boundedPseudonyms.has(decodedBoundedPseudonym)).toEqual(true);
    expect(pres1.spec.boundedPseudonyms.get(decodedBoundedPseudonym)).toEqual({
      commitKey: {
        basesForAttributes: basesForAttributes,
        baseForSecretKey: baseForSecretKey
      },
      attributes: attributeNames1
    });
    ...
    expect(pres1.spec.unboundedPseudonyms.size).toEqual(1);
    expect(pres1.spec.unboundedPseudonyms.has(decodedUnboundedPseudonym)).toEqual(true);
    expect(pres1.spec.unboundedPseudonyms.get(decodedUnboundedPseudonym)).toEqual({
      commitKey: {
        baseForSecretKey: baseForSecretKey
      }
    });
